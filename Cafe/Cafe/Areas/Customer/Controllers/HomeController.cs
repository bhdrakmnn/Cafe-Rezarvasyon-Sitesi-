    ...
    { 
        ...
        public async Task<IActionResult> Contact([Bind("Id,Name,Email,Telefon,Mesaj,")] Contact contact)
        {
            if (ModelState.IsValid)
            {
                ...
                _toast.AddSuccessToastMessage("Teşekkürler, Mesajınız başarıyla iletildi...");
        ...
        {
            if (ModelState.IsValid)
                blog.Tarih = DateTime.Now; //hangi tarihte yorum yapıldıysa otomatik gelecek.
            {
                ...
                _db.Add(blog);
                await _db.SaveChangesAsync();
                _toast.AddSuccessToastMessage("Teşekkürler, Yorumunuz iletildi, Yorumunuz onaylandığında yorumlar sayfasında görebilirsiniz...");
                return RedirectToAction(nameof(Index));
            }
            return View(blog);
        }
        ...
        public async Task<IActionResult> Rezervasyon([Bind("Id,Name,Email,TelefonNo,Sayi,Saat,Tarih")] Rezervasyon rezervasyon)
        {
            if (ModelState.IsValid)
            {
                _db.Add(rezervasyon);
                await _db.SaveChangesAsync();
                _toast.AddSuccessToastMessage("Rezervasyon işleminiz başarılıyla gerçekleştirilmiştir...");
                return RedirectToAction(nameof(Index));
            }
